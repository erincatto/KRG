...
namespace KRG::TypeSystem::Reflection
{
    ...
    bool ReflectionDatabase::Disconnect()
    {
        if ( m_pDatabase != nullptr )
        {
            ...
            KRG_ASSERT( result == SQLITE_OK ); // If we get SQLITE_BUSY, this means we are leaking sqlite resources
            m_pDatabase = nullptr;
    ...
    bool ReflectionDatabase::IsTypeDerivedFrom( TypeID typeID, TypeID parentTypeID ) const
    {
        ...
        KRG_ASSERT( pTypeDesc != nullptr ); // Unknown Type
        KRG_ASSERT( IsTypeRegistered( parentTypeID ) ); // Unknown Type
    ...
    KRG::TypeSystem::Reflection::ReflectedProperty const* ReflectionDatabase::GetPropertyTypeDescriptor( TypeID typeID, PropertyPath const& pathID ) const
    {
        ...
        for ( size_t i = 0; i < numPathElements; i++ )
        {
            ...
            if ( i != lastElementIdx )
            {
                ...
                KRG_ASSERT( !IsCoreType( pResolvedPropertyTypeDesc->m_typeID ) && !pResolvedPropertyTypeDesc->IsEnumProperty() );
                pCurrentTypeDesc = GetType( pResolvedPropertyTypeDesc->m_typeID );
                if ( pCurrentTypeDesc == nullptr )
                {
                    KRG_LOG_ERROR( "Reflection::ReflectionDatabase", "Cant resolve property path since it contains an unknown type" );
                    return nullptr;
                }
            }
        }
    ...
    bool ReflectionDatabase::CreateTables()
    {
        ...
        if ( !ExecuteSimpleQuery( "CREATE TABLE IF NOT EXISTS `EnumConstants` ( `TypeID` INTEGER NOT NULL, `Label` TEXT NOT NULL, `Value` INTEGER, PRIMARY KEY(TypeID, Label) );" ) )
        {
            return false;
        }
    ...
    bool ReflectionDatabase::ReadAdditionalEnumData( ReflectedType& type )
    {
        ...
        sqlite3_stmt* pStatement = nullptr;
        FillStatementBuffer( "SELECT `Label`, `Value` FROM `EnumConstants` WHERE `EnumConstants`.TypeID = %u;", (uint32_t) type.m_ID );
        if ( IsValidSQLiteResult( sqlite3_prepare_v2( m_pDatabase, m_statementBuffer, -1, &pStatement, nullptr ) ) )
        {
            while ( sqlite3_step( pStatement ) == SQLITE_ROW )
            {
    ...
    bool ReflectionDatabase::WriteAdditionalEnumData( ReflectedType const& type )
    {
        ...
        {
            if ( !ExecuteSimpleQuery( "INSERT INTO `EnumConstants`(`TypeID`,`Label`,`Value`) VALUES ( %u, \"%s\", %u );", (uint32_t) type.m_ID, enumConstant.second.m_label.c_str(), enumConstant.second.m_value ) )
            {
                return false;
            }
        }
