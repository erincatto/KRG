        ...
        Milliseconds time = 0;
        {
            ...
            for ( auto& prj : m_solution.m_projects )
            {
                ...
                for ( auto i = 0u; i < prj.m_headerFiles.size(); i++ )
                {
                    ...
                    FileSystem::Path const autoGeneratedTargetHeaderFilePath = header.GetAutogeneratedHeaderFileName( autoGeneratedDirectory );
                    FileSystem::Path const autoGeneratedTargetCodeFilePath = header.GetAutogeneratedImplementationFileName( autoGeneratedDirectory );

                    isDirty = !FileSystem::Exists( autoGeneratedTargetHeaderFilePath ) || !FileSystem::Exists( autoGeneratedTargetCodeFilePath );
                    ...
                    {
                        if ( existingFiles[j].IsFilenameEqual( autoGeneratedTargetHeaderFilePath.GetFileNameWithoutExtension() ) )
                        {
