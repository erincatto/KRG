...
namespace KRG::CPP
{
    ...
    static void GenerateStaticTypeRegistrationMethod( std::stringstream& file, ReflectedType const& type )
    {
        ...
        {
            file << "                IRegisteredType* pDefaultTypeInstance = KRG::New<" << type.m_namespace.c_str() << type.m_name.c_str() << ">();\n";
        }

        file << "                " << type.m_namespace.c_str() << type.m_name.c_str() << "::s_pTypeInfo = KRG::New<TTypeInfo<" << type.m_namespace.c_str() << type.m_name.c_str() << ">>( pDefaultTypeInstance );\n";
    ...
    static void GenerateStaticTypeUnregistrationMethod( std::stringstream& file, ReflectedType const& type )
    {
        ...
        if ( !type.IsAbstract() )
        {
            file << "                KRG::Delete( const_cast<IRegisteredType*&>( " << type.m_namespace.c_str() << type.m_name.c_str() << "::s_pTypeInfo->m_pDefaultInstance ) );\n";
        }

        file << "                KRG::Delete( " << type.m_namespace.c_str() << type.m_name.c_str() << "::s_pTypeInfo );\n";
        file << "            }\n\n";
    }
