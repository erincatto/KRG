...
namespace KRG::Resource
{
    ...
    ResourceServer::~ResourceServer()
    {
        KRG_ASSERT( m_pCompilerRegistry == nullptr );
    }
    ...
    bool ResourceServer::Initialize( IniFile const& iniFile )
    {
        ...
        m_pCompilerRegistry = KRG::New<CompilerRegistry>( m_typeRegistry, m_settings.m_rawResourcePath );
        ...
        {
            m_workers.emplace_back( KRG::New<ResourceServerWorker>( &m_taskSystem, m_settings.m_resourceCompilerExecutablePath.c_str() ) );
        }
    ...
    void ResourceServer::Shutdown()
    {
        ...
        KRG::Delete( m_pCompilerRegistry );
    ...
    ResourceServer::BusyState ResourceServer::GetBusyState() const
    {
        ...
        if ( state.m_isBusy )
        {
            KRG_ASSERT( state.m_totalRequests > 0 );
            KRG_ASSERT( state.m_completedRequests <= state.m_totalRequests );
        }
    ...
    void ResourceServer::NotifyClientOnCompletedRequest( CompilationRequest* pRequest )
    {
        ...
        if ( pRequest->IsInternalRequest() )
        {
            ...
            // Note: We only send the notification if the request was successful
            if ( pRequest->HasSucceeded() )
            {
    ...
    {
        KRG_ASSERT( !m_isPackaging && m_resourcesToBePackaged.empty() );
        m_completedPackagingRequests.clear();
