...
namespace KRG::Animation
{
    ...
    {
        KRG_ASSERT( m_pose.GetSkeleton() == rhs.m_pose.GetSkeleton() );
    ...
    UUID PoseBufferPool::CreateCachedPoseBuffer()
    {
        ...
        if ( m_firstFreeCachedBuffer == m_cachedBuffers.size() )
        {
            ...
            KRG_ASSERT( m_cachedBuffers.size() < 255 );
        }
        else
        {
            pCachedPoseBuffer = &m_cachedBuffers[m_firstFreeCachedBuffer];
            KRG_ASSERT( !pCachedPoseBuffer->m_isUsed );
        }
    ...
    {
        KRG_ASSERT( InCachedPoseID.IsValid() );
    ...
    {
        KRG_ASSERT( cachedPoseID.IsValid() );
        ...
        KRG_ASSERT( pFoundCachedPoseBuffer != nullptr );
    ...
    void PoseBufferPool::RecordPose( int8_t poseBufferIdx )
    {
        ...
        if ( m_firstFreeDebugBuffer == m_debugBuffers.size() )
        {
            ...
            KRG_ASSERT( m_debugBuffers.size() < 255 );
        }

        KRG_ASSERT( m_poseBuffers[poseBufferIdx].m_isUsed );
