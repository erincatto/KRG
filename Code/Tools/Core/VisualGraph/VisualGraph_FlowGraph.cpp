...
namespace KRG::VisualGraph::Flow
{
    ...
    UUID Node::RegenerateIDs( THashMap<UUID, UUID>& IDMapping )
    {
        ...
        for ( auto& pin : m_inputPins )
        {
            ...
            KRG_ASSERT( IDMapping.find( originalPinID ) == IDMapping.end() );
...
namespace KRG::VisualGraph
{
    ...
    void FlowGraph::SerializeCustom( TypeSystem::TypeRegistry const& typeRegistry, RapidJsonValue const& graphObjectValue )
    {
        ...
        for ( auto& connectionObjectValue : graphObjectValue[s_connectionsKey].GetArray() )
        {
            ...
            KRG_ASSERT( connection.m_pStartNode != nullptr && connection.m_pEndNode != nullptr );
        }
    }
    ...
    UUID FlowGraph::RegenerateIDs( THashMap<UUID, UUID>& IDMapping )
    {
        ...
        for ( auto& connection : m_connections )
        {
            ...
            KRG_ASSERT( IDMapping.find( originalConnectionID ) == IDMapping.end() );
