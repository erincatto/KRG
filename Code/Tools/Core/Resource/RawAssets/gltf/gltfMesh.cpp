...
namespace KRG::RawAssets
{
    ...
    public:
        ...
        {
            KRG_ASSERT( IsValidTriangleMesh( meshData ) );
            ...
            for ( auto p = 0; p < meshData.primitives_count; p++ )
            {
                ...
                KRG_ASSERT( primitive.attributes_count > 0 );
                ...
                for ( auto a = 0; a < primitive.attributes_count; a++ )
                {
                    switch ( primitive.attributes[a].type )
                    {
                        ...
                        {
                            KRG_ASSERT( primitive.attributes[a].data->type == cgltf_type_vec2 );
            ...
            if ( sceneCtx.IsValid() )
            {
                ...
                else // Read skeletal mesh data
                {
                    ...
                    for ( auto l = 0; l < skins[0].m_pSkin->joints_count; l++ )
                    {
                        ...
                        {
                            pRawMesh->LogError( "Unnamed joint (%d) encountered, this is not supported by the KRG importer: %s -> %s", l, sourceFilePath.c_str(), sceneCtx.GetErrorMessage().c_str() );
                            return pMesh;
                        }
                    ...
                    KRG_ASSERT( skins[0].m_pSkin->joints_count > 0 );
