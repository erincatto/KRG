...
namespace KRG
{
    ...
    namespace Profiling
    {
        KRG_SYSTEM_API void StartFrame();
        KRG_SYSTEM_API void EndFrame();

        //-------------------------------------------------------------------------

        // Open the profiler application (only available on Win64)
        KRG_SYSTEM_API void OpenProfiler();

        // Capture management
        KRG_SYSTEM_API void StartCapture();
        KRG_SYSTEM_API void StopCapture( FileSystem::Path const& captureSavePath );
    }
}
