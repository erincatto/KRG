...
namespace KRG::Serialization
{
    ...
    void BinaryReader::BeginReading( char const* pData, size_t size )
    {
        ...
        KRG_ASSERT( m_pReader == nullptr );
        m_pReader = KRG::New<mpack_reader_t>();
    ...
    {
        KRG_ASSERT( m_pWriter == nullptr );
        m_pWriter = KRG::New<mpack_writer_t>();
    ...
    void BinaryWriter::WriteValue( StringID const& v )
    {
        ...
        {
            mpack_write_cstr_or_nil( m_pWriter, "" );
        }
    }
    ...
    bool BinaryInputArchive::ReadFromFile( FileSystem::Path const& filePath )
    {
        ...
        if ( FileSystem::Exists( filePath ) )
        {
            ...
            m_pFileData = KRG::Alloc( m_fileDataSize );
