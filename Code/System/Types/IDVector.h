...
namespace KRG
{
    ...
    public:
        ...
        void Remove( IDType const& ID )
        {
            ...
            KRG_ASSERT( foundIter != m_indexMap.end() );
        ...
        {
            auto pFoundItem = FindItem( ID );
            KRG_ASSERT( pFoundItem != nullptr );
            return pFoundItem;
        }

    private:

        KRG_FORCE_INLINE IDType GetItemID( std::true_type, ItemType const& pItem ) const
        {
            return pItem->GetID();
        }

        KRG_FORCE_INLINE IDType GetItemID( std::false_type, ItemType const& item ) const
        {
            return item.GetID();
        }

        KRG_FORCE_INLINE IDType GetLastElementID( std::true_type ) const
        {
            KRG_ASSERT( !m_vector.empty() );
            return m_vector.back()->GetID();
        }

        KRG_FORCE_INLINE IDType GetLastElementID( std::false_type ) const
        {
            KRG_ASSERT( !m_vector.empty() );
            return m_vector.back().GetID();
        }
