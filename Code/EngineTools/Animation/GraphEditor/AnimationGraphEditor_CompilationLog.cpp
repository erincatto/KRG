...
namespace KRG::Animation
{
    ...
    void GraphCompilationLog::UpdateAndDraw( UpdateContext const& context, DebugContext* pDebugContext, ImGuiWindowClass* pWindowClass, char const* pWindowName )
    {
        ...
        if ( ImGui::Begin( pWindowName, nullptr, windowFlags ) )
        {
            ...
            {
                ImGui::TextColored( ImGuiX::ConvertColor( Colors::LimeGreen ), KRG_ICON_CHECK );
                ImGui::SameLine();
                    ...
                    while ( clipper.Step() )
                    {
                        for ( int i = clipper.DisplayStart; i < clipper.DisplayEnd; i++ )
                        {
                            ...
                            switch ( entry.m_severity )
                            {
                                ...
                                ImGui::TextColored( Colors::Yellow.ToFloat4(), KRG_ICON_ALERT_OCTAGON );
                                break;

                                case Log::Severity::Error:
                                ImGui::TextColored( Colors::Red.ToFloat4(), KRG_ICON_ALERT );
                                break;
