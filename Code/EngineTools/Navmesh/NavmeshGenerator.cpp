...
namespace KRG::Navmesh
{
    ...
    {
        KRG_ASSERT( rawResourceDirectoryPath.IsValid() );
        KRG_ASSERT( m_outputPath.IsValid() );
    }

    NavmeshGenerator::~NavmeshGenerator()
    {
        KRG_ASSERT( m_pNavpowerInstance == nullptr );
        KRG_ASSERT( m_state != State::Generating );
        KRG_ASSERT( !m_isGeneratingAsync );
    }
    ...
    {
        KRG_ASSERT( m_state != State::Generating && m_pNavpowerInstance == nullptr );
    ...
    bool NavmeshGenerator::CollectTriangles()
    {
        ...
        for ( auto const& primitiveDesc : m_collisionPrimitives )
        {
            ...
            { 
                [this] ( char const* pString ) { KRG_LOG_WARNING( "Navmesh Generator", pString ); },
                [this] ( char const* pString ) { KRG_LOG_ERROR( "Navmesh Generator", pString ); }
            };
